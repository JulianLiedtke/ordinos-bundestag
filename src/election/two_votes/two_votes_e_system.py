...
class FirstVote():
    ...
    def add_random_votes(self, n):
        ...
        for cand in self.cands:
            ...
            log.info("Kandidat: " + str(cand) + " Stimmen: " + str(rand_vote))
...
class TwoVotesESystem():
    ...
    def __init__(self, bits_key, bits_int, n_trustees, threshold, n_parties, secret_residual=False):
        ...
        # FIXME pass the bulletin board
        self.trustees = init_trustees(BulletinBoard(), self.pk, sk_shares)
        self.init_votes()
    ...
    def run(self):
        ...
        log.info("Mit Erststimmen gewählte Kandidaten: " + str(winner_first_votes))
        ...
        for id in winner_first_votes:
            ...
            if party not in self.clause_exclusions:
                ...
                log.info("Partei " + str(party) + " zu Sperrklausel-exclusions hinzugefügt")
        ...
        sum_seats = 0
        for party in range(self.n_parties):
            log.info("Partei " + str(party))
            if party in seats:
                sum_seats += seats[party]
                log.info("Sitze " + str(seats[party]))
            else:
                log.info("Unter Sperrklausel")
            log.info("--------------------------")
        log.info("Parteien gewonner Restsitze " + str(residual))
        log.info("--------------------------")
        ...
        if sum_seats != self.num_seats:
            ...
            log.info("Sitzzahl aufgrund Gleichstand verändert auf " + str(self.num_seats) + " Sitze")

        #execute third step (compute balanced seat distribution)
        balance = self.balance(seats, first_votes_party, 1, True)
        for party, new_seats in balance.items():
            log.info("Partei " + str(party))
            log.info("Balancierte Zweitstimmen-Sitzzahl " + str(new_seats))
            log.info("--------------------------")
...
class CSV_Writer_two_votes:
    ...
    def init_writer(cls):
        # TODO: path of files
        cls.fieldnames = ['Sitze', 'Parteien', 'Geheim?', 'Gescheiterte Parteien', 'Restsitze?', 'Zeit 1', 'Zeit 2']
        cls.csvfile = open('times.csv', 'w', newline='')
