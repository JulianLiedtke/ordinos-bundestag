...
from src.election.single_vote.single_vote_election_system import \
    SingleVoteElection
from src.election.trustee.trustee import Trustee, init_trustees
...
class CondorcetElectionTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # abbs = PaillierABB.gen_trustee_abbs(64, 5, 3, SubLinearProtocolSuite) #to run with Paillier
    ...
    def test_condorcet_matrix_one(self):
        self._condorcet_one(ElectionAuthority(self.trustee_gen, Condorcet(4), local_bulletin_board=True))

    def test_condorcet_matrix_two(self):
        result = self._condorcet_two(ElectionAuthority(self.trustee_gen, Condorcet(4), local_bulletin_board=True))
        self.assertEqual(0, len(result))

    def test_condorcet_matrix_three(self):
        self._condorcet_three(ElectionAuthority(self.trustee_gen, Condorcet(4), local_bulletin_board=True))

    def test_condorcet_matrix_leak_one(self):
        self._condorcet_one(ElectionAuthority(self.trustee_gen, Condorcet(4, leak_better_half=False), local_bulletin_board=True))

    def test_condorcet_matrix_leak_two(self):
        result = self._condorcet_two(ElectionAuthority(self.trustee_gen, Condorcet(4, leak_better_half=False), local_bulletin_board=True))
        self.assertEqual(0, len(result))

    def test_condorcet_matrix_leak_three(self):
        self._condorcet_three(ElectionAuthority(self.trustee_gen, Condorcet(4, leak_better_half=False), local_bulletin_board=True))

    def test_condorcet_matrix_wikipedia(self):
        result = self._condorcet_wikipedia(ElectionAuthority(self.trustee_gen, Condorcet(4), local_bulletin_board=True))
        self.assertEqual(0, len(result))
        return result

    def test_minimax_margins(self):
        result = self._condorcet_wikipedia(ElectionAuthority(self.trustee_gen, MiniMaxMarginsSmith(4), local_bulletin_board=True))
        self.assertEqual(1, len(result))
        self.assertEqual(1, result[0])

    def test_minimax_winning_votes(self):
        result = self._condorcet_wikipedia(ElectionAuthority(self.trustee_gen, MiniMaxWinningVotesSmith(4), local_bulletin_board=True))
        self.assertEqual(1, len(result))
        self.assertEqual(0, result[0])

    def test_minimax_copeland(self):
        result = self._condorcet_wikipedia(ElectionAuthority(self.trustee_gen, Copeland(4), local_bulletin_board=True))
    ...
    def test_minimax_copeland_leak(self):
        result = self._condorcet_wikipedia(ElectionAuthority(self.trustee_gen, Copeland(4, leak_max_points=True), local_bulletin_board=True))
        self.assertEqual(3, len(result))
        self.assertTrue(0 in result)
        self.assertTrue(1 in result)
        self.assertTrue(2 in result)
    ...
    def test_minimax_copeland_leak_borda(self):
        result = self._condorcet_wikipedia(ElectionAuthority(self.trustee_gen, Copeland(4, leak_better_half=True, leak_max_points=True), local_bulletin_board=True))
        self.assertEqual(3, len(result))
        self.assertTrue(0 in result)
        self.assertTrue(1 in result)
        self.assertTrue(2 in result)
    ...
    def test_weak_condorcet(self):
        result = self._condorcet_weak(ElectionAuthority(self.trustee_gen, Condorcet(4, [WeakCondorcetEvaluation]), local_bulletin_board=True))
        self.assertEqual(1, len(result))
        self.assertTrue(0 in result)

    def test_schulze(self):
        result = self._condorcet_two(ElectionAuthority(self.trustee_gen, Condorcet(4, additional_evaluators=[SchulzeEvaluation]), local_bulletin_board=True))
