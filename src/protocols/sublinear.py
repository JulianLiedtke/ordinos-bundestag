...
class SublinearMultiplicationProtocol(Protocol):
    ...
    def run(self, enc_x, enc_y):
        ...
        s = self.abb.dec(enc_s)   # TODO: counted as decryption call in top level; alternatively remove decryption counting because it is very fast
...
class BroadcastRandomMultProtocol(Protocol):
    ...
    def run(self, enc_y):
        ...
        serializedProof = {
            "announcement": {
                "a": proof[0][0].serialize(),
                "b": proof[0][1].serialize()
            },
            "challenge": str(proof[1]),
            "response": {
                "d": str(proof[2][0]),
                "e": str(proof[2][1]),
                "f": str(proof[2][2])
        ...
        enc_d_list = [(id, self.abb.deserialize_cipher(ser)) for id, ser in self.broadcast_and_receive(enc_d.serialize())]
        enc_e_list = [(id, self.abb.deserialize_cipher(ser)) for id, ser in self.broadcast_and_receive(enc_e.serialize())]
        zk_proofs =  [(id, ((self.abb.deserialize_cipher(ser["announcement"]["a"]),
                             self.abb.deserialize_cipher(ser["announcement"]["b"])),
                             mpz(ser["challenge"]),
                            (mpz(ser["response"]["d"]),
                             mpz(ser["response"]["e"]),
                             mpz(ser["response"]["f"])))) for id, ser in self.broadcast_and_receive(serializedProof)]
...
class SublinearStorage():
    ...
    def get_data(self, bits_int):
        ...
        try:
            ...
            log.info("Fehler ist hier im try. Meisten muss man einfach die Verschlüsslungsbits erhöhen.")
        finally:
            self.lock.release()
        return result
...
class GtStorage(SublinearStorage):
    ...
    def _generate_data(self, filename, bits_int):
        ...
        # TODO parties should choose randomness
