...
class PlainABB(ABB):
    ...
    def deserialize_cipher(self, serialized_cipher):
        return PlainCiphertext.deserialize(self, serialized_cipher)
    ...
    def deserialize(cls, serialized):
        pk = PublicPlainKey.deserialize(serialized['pk'])
        sk = SecretPlainKey.deserialize(serialized['sk']) if serialized['sk'] is not None else None
...
class PublicPlainKey():
    ...
    @property
    def n(self):
        return mpz(self.get_max_plaintext_value())
...
class SecretPlainKey():
    ...
    @classmethod
    def deserialize(cls, serialized):
        return cls(serialized["index"], serialized["threshold"])
