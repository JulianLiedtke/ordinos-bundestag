...
class CondorcetEvaluation(EvaluationProtocol):
    ...
    def run(self, vote_aggregation):
        ...
        if self.leak_better_half:
            ...
            possible_winner = self.run_subprotocol(PointThresholdEvaluation(self.n_cand * self.n_votes * 2, self.abb.enc_no_r(int(math.floor(average_borda_points * self.n_votes)))), [vote_aggregation[1]])
        else:
